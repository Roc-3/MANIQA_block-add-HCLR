...
class SeedGenerater(nn.Module):
    ...
    def seed_generate(self, spix):
        ...
        sp_c = torch.from_numpy(np.array(sp_c)).long().to(device)  # 确保 sp_c 在同一个设备上

        o_cind = sp_c
        o_cx = torch.floor(o_cind / float(w))
        o_cy = torch.floor(o_cind - o_cx * w)
        o_cx = o_cx.to(device)  # 确保 o_cx 在同一个设备上
        o_cy = o_cy.to(device)  # 确保 o_cy 在同一个设备上
    ...
    def grid_seed(self, spix):
        ...
        S = h * w / self.sp_num
        sp_h = np.int32(np.floor(np.sqrt(S) / (w /  np.float64(h))))
        sp_w = np.int32(np.floor(S / np.floor(sp_h)))
