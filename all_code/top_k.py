...
class Attention(nn.Module):
    ...
    def forward(self, x, x_texture):
        ...
        # 多头分离
        q = self.q_dwconv(self.q_proj(x_texture))
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type):
        ...
        self.norm1_x_texture = LayerNorm(dim, LayerNorm_type)
        self.attn = Attention(dim, num_heads, bias)
        self.norm2 = LayerNorm(dim, LayerNorm_type)
        self.ffn = FeedForward(dim, ffn_expansion_factor, bias)

    def forward(self, x, x_texture):
        # 对 x 和 x_texture 分别归一化
        norm_x = self.norm1_x(x)
        norm_x_texture = self.norm1_x_texture(x_texture)
        
        # 注意力融合
        x = x + self.attn(norm_x, norm_x_texture)
