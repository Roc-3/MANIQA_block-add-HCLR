...
from keras.layers.convolutional import Convolution2D, MaxPooling2D,ZeroPadding2D #, UpSampling2D ,Deconvolution2D,AveragePooling2D
...
def M_ResNet50nob(input_tensor=None, train_bn=None):
    ...
    # Load weights
    # model.load_weights('code/DINet/models/resnet50.h5')
...
def DINetDecoder(img_rows=480, img_cols=640):   
    ...
    final_output_up = Lambda(lambda x: tf.image.resize(x, size=(int(x.shape[1])*8, int(x.shape[2])*8), method=tf.image.ResizeMethod.BILINEAR))(final_output)
